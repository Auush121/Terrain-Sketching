...
void SilhouetteMode::noisePropagate()
{
    ...
    // Reduce the variance for noise reduction (e.g., make it 10% of the original)
    float var = std::sqrt(current_stroke->getSilhouetteCurve()->getVariance());
    var *= 0.1f;  // Scaling down the variance for noise reduction (e.g., 10% of the original)

    std::normal_distribution<> d(0, var);  // Create normal distribution with reduced variance
    ...
    for (int i = 0; i < max_terrain_points; i++)
    {
        ...
        // Clamp the deformation to avoid large jumps
        del_terrain = glm::clamp(del_terrain, -0.5f, 0.5f);  // Clamp deformation within a reasonable range
